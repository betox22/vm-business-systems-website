...
const departments = [
  ...
  { name: "Deals", desc: "Daily store offers", count: "9K", visual: "sticker" }
];
  ...
  return `
    <article class="product-card ${extraClass}">
      ...
      <div class="product-info">
        ...
        <h3>${product.name}</h3>
        <div class="vendor-line">By ${product.brand}</div>
...
function renderHome() {
  ...
  return `
    <section class="trust-strip" aria-label="Store trust signals">
      <span><b></b> Protected checkout</span>
      <span><b></b> Fast store shipping</span>
      <span><b></b> Owned inventory</span>
    ...
    <section class="commerce-hero">
      <div class="hero-main">
        <div class="hero-copy">
          <span class="eyebrow">Daily deals from one curated catalog</span>
          <h1>Shop smarter across every department.</h1>
          <p>Discover new drops, compare categories, and check out through one clean store-owned shopping flow.</p>
          <div class="hero-value-row" aria-label="Buyer benefits">
            <span>Fast shipping picks</span>
            <span>Buyer protection</span>
            <span>One store checkout</span>
          </div>
          <div class="hero-actions">
            <button class="primary-button" type="button" data-route="catalog">Shop catalog</button>
    ...
    <section>
      <div class="section-head">
        <div>
          <h2>Departments</h2>
          <p>Fast paths for a broad retail catalog managed by one business.</p>
        </div>
  ...
  return `
    <section class="catalog-page">
      <div class="catalog-hero">
        <div>
          <span class="catalog-kicker">Retail catalog</span>
          <h2>Explore every category in one store.</h2>
          <p>High-density browsing with fast filters, category trust, delivery signals and cart-ready cards.</p>
          <div class="catalog-pill-row" aria-label="Active catalog filters">
            ...
            <button type="button">Store picks</button>
          </div>
        </div>
      ...
      <section class="catalog-layout">
        <aside class="filter-panel catalog-filter-panel">
          ...
          <div class="filter-group">
            <strong>Store trust</strong>
            <label class="filter-check"><input type="checkbox" checked> Store verified <span>860</span></label>
        ...
        <div class="catalog-results">
          <div class="results-toolbar catalog-toolbar">
            <div>
              <strong>${products.length} store results</strong>
              <span>Owned catalog · protected checkout · inventory-ready totals</span>
            </div>
          ...
          <div class="catalog-alert">
            ...
            <span>Final totals are validated by the backend before payment.</span>
          </div>
    ...
    <section class="product-page">
      ...
      <section class="product-detail">
        ...
        <div class="product-info-panel product-story-panel">
          ...
          <p>${product.desc} Compare category fit, delivery speed, returns and final quote before checkout.</p>

          <div class="product-highlight-grid">
            <article><strong>Ships fast</strong><span>${product.shipping} from owned store inventory.</span></article>
            <article><strong>Quote protected</strong><span>Backend validates price, stock and tax before payment.</span></article>
            <article><strong>Buyer trust</strong><span>Ratings, returns and product status remain visible.</span></article>
          </div>
          ...
          <div class="seller-card">
            <div class="seller-avatar">${product.brand.slice(0, 1)}</div>
            <div>
              <strong>${product.brand}</strong>
              <span>Store collection · 98% on-time fulfillment</span>
            </div>
        ...
        <aside class="product-buy-box product-purchase-card">
          ...
          <p class="shipping">Free returns on eligible products. Final total is confirmed before payment.</p>
      ...
      <section class="related-products">
        <div class="section-head">
          <div>
            ...
            <p>Comparable products keep the buyer in the store discovery flow.</p>
          </div>
        </div>
...
function cartGroups() {
  ...
  return [["Kreaton owned inventory", items]];
}
    ...
    <section class="cart-page">
      <div class="cart-hero">
        <div>
          ...
          <p>One customer checkout validates one owned-inventory order behind the scenes.</p>
        </div>
          ...
          <article class="cart-group">
            <div class="cart-group-header">
              <div>
                ...
                <span>${items.length} item${items.length === 1 ? "" : "s"} · single store order</span>
              </div>
              <span class="badge">owned inventory</span>
            </div>
            ${items.map((item) => `
              <div class="cart-item">
      ...
      <aside class="summary-panel checkout-summary-card">
        ...
        <div class="secure-list">
          ...
          <span><b></b> Inventory validation server-side</span>
    ...
    <section class="checkout-page">
      <div class="checkout-hero">
        <div>
          ...
          <h2>One payment, one clean store order.</h2>
          <p>The storefront feels simple for the buyer while the backend validates inventory, tax, shipping and payment readiness.</p>
        </div>
        ...
        <article class="checkout-step">
          ...
          <div class="checkout-step-body">
            <div class="payment-options">
              ...
              <button type="button"><strong>Invoice</strong><span>B2B eligible orders</span></button>
            </div>
            <div class="status-note">Payment UI remains isolated. Place order requires an Idempotency-Key and fresh backend quote.</div>
          </div>
        </article>
        <article class="checkout-step">
          <div class="checkout-step-header"><strong>3. Review order</strong><span class="badge">quote ready</span></div>
          <div class="checkout-step-body">
            ${groups.map(([groupName, items]) => `
              <div class="vendor-split-line">
                <div><strong>${groupName}</strong><span>${items.length} item${items.length === 1 ? "" : "s"} · inventory reserved after quote</span></div>
      ...
      <aside class="summary-panel checkout-summary-card">
        ...
        <div class="secure-list">
          ...
          <span><b></b> Order record created after capture</span>
        </div>
      </aside>
    </section>
    </section>
