...
## 2026-09-11 — Codex — Formulario real de solicitudes y respaldo por email

**Hecho:** se reemplazó el mockup `.form-preview-card` de `contact.html` por un
formulario accesible y responsive con validación cliente/servidor, estados visibles
de carga, éxito y error, referencia `SOL-*`, honeypot anti-spam y fallback de correo.
El CTA final y el CTA del hero ahora llevan al formulario real. Se agregó
`POST /api/v1/leads`, una tabla SQLAlchemy `leads` completamente aditiva y una
notificación secundaria por Resend vía HTTP. La fila se guarda y confirma antes de
intentar el email; si Resend falta o falla, el lead permanece registrado y el estado
queda auditado en `notification_status`. `public-dist/contact.html` y sus assets son
generados por `scripts/stage-public-site.mjs`; no se editaron a mano. No existe
`dist/contact.html` en este checkout.

**Decisión técnica:** Resend evita otro SDK y usa `httpx`, ya existente. Las variables
son `RESEND_API_KEY`, `LEADS_NOTIFY_EMAIL` y `LEADS_FROM_EMAIL`. Ninguna dirección
inventada quedó hardcodeada como destino del backend. Se omitió la vista admin de
leads porque era opcional y habría ampliado la superficie del panel/autorización;
el objetivo comercial crítico —capturar y notificar— quedó completo.

**Validación:** 12/12 pruebas dirigidas de leads + Billing/commerce pasaron; el flujo
real `contact.html -> POST /api/v1/leads -> confirmación` se ejecutó en navegador
contra Uvicorn y SQLite temporal. Referencia observada `SOL-4BD1C493C924`; la fila
quedó como `Negocio QA Local / not_configured` y el log confirmó que el email se
omitió por falta de variables sin perder el lead. Validación requerida mostró
`aria-invalid=true`; consola sin errores/advertencias durante el flujo exitoso.
La suite completa del backend terminó con 129 tests, OK.

**Pendiente / abierto:** configurar en Render un remitente verificado de Resend y
una bandeja administrativa real, luego repetir el envío E2E para confirmar delivery.
La vista admin de solo lectura sigue opcional. Los textos históricos sobre correos y
representantes futuros no se modificaron porque la tarea prohibía reescribir esas
secciones fuera del formulario.

**Archivos tocados:** `contact.html`, `css/styles.css`, `js/contact-leads.js`,
`backend/app/leads.py`, `backend/app/db_models.py`, `backend/app/main.py`,
`backend/.env.example`, `backend/tests/test_leads.py`, `docs/AGENT_LOG.md`.

**Notas para el siguiente agente:** mantener el orden persistir-antes-de-notificar.
No convertir un fallo de Resend en error HTTP después de que el lead ya se guardó.
Para producción Postgres, incluir `leads` en la migración formal del mismo deploy.
